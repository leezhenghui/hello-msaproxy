...
class Stub {
	...
	static get SVC_DOMAIN_NAME_POSTFIX() {
    return '-ref.service.intra';	
	}

  constructor(svcname) {
		this.addr= svcname + Stub.SVC_DOMAIN_NAME_POSTFIX;
  }
...
class Operator {
    ...
    if (! op) throw new Error('Invalid operator: "' + op + '"');	
		if ('+' === op) {
	    this.opInvoker = new Stub('add-svc');	
		} else if ('-' === op) {
	    this.opInvoker = new Stub('sub-svc');	
		} else {
	...
	execute(l, r) {
		let path = '/api/v1/execute/?l=' + l + '&r=' + r;
